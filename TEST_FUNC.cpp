...
extern "C" __declspec(dllexport) double HillFunc(double x, time_t now) {
  ...
  unsigned short i = 1;
  while (i < HH_COUNT) {
    res += (-1.1 + distribution(generator) * 2) * sin(M_PI * i * x * 2) +
           (-1.1 + distribution(generator) * 2) * cos(M_PI * i * x * 2);
    i++;
  }
  return res;
}
extern "C" __declspec(dllexport) double ShekelFunc(double x, time_t now) {
  ...
  unsigned short i = 0;
  while (i < SH_COUNT) {
    res -= 1 / ((5 + 20 * distribution(generator)) *
                    pow(x - 10 * distribution(generator), 2) +
                1 + 0.4 * distribution(generator));
    i++;
  }
  return res;
}
extern "C" __declspec(dllexport) double GrishaginFunc(double x1, double x2,
  ...
  while (i < GR_COUNT) {
    ...
    while (j < GR_COUNT) {
      part1 +=
          distribution(generator) * sin(M_PI * i * x1) * sin(M_PI * j * x2) +
          distribution(generator) * cos(M_PI * i * x1) * cos(M_PI * j * x2);
      part2 +=
          distribution(generator) * sin(M_PI * i * x1) * sin(M_PI * j * x2) -
          distribution(generator) * cos(M_PI * i * x1) * cos(M_PI * j * x2);
      j++;
...
extern "C" __declspec(dllexport) double RastriginFunc(double x1, double x2) {
  return (x1 * x1 + x2 * x2 - 10 * (cos(2 * M_PI * x1) + cos(2 * M_PI * x2)) +
          14.6) *
         (18.42 - 0.25 * (x1 * x1 + x2 * x2));
}
